...
namespace VulkanCookbook {
    ...
    bool CheckAvailableDeviceExtensions(VkPhysicalDevice physical_device,
        ...
        if ((result != VK_SUCCESS) ||
            (extensions_count == 0)) {
            std::cout << "Could not get the number of device availableExtensions." << std::endl;
            return false;
        }
        ...
        if ((result != VK_SUCCESS) ||
            (extensions_count == 0)) {
            std::cout << "Could not enumerate device availableExtensions." << std::endl;
            return false;
        }
...
#define DEVICE_LEVEL_VULKAN_FUNCTION(name)                                    \
    ...
    if( name == nullptr ) {                                                     \
      ...
      return false;                                                             \
        ...
        // Load device-level functions from enabled availableExtensions
